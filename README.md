...
# Expert settings
CMANGOS_REALMD_CONFIG_TEMPLATE_OVERRIDE: false  # Custom Jinja template path for realmd.conf; uses the built-in template when false.
CMANGOS_MANGOSD_CONFIG_TEMPLATE_OVERRIDE: false  # Custom Jinja template path for mangosd.<expansion>.conf; uses the built-in template when false.
CMANGOS_AHBOT_CONFIG_TEMPLATE_OVERRIDE: false  # Custom Jinja template path for ahbot.conf; uses the built-in template when false.
CMANGOS_AIPLAYERBOT_CONFIG_TEMPLATE_OVERRIDE: false  # Custom Jinja template path for aiplayerbot.<expansion>.conf; uses the built-in template when false.
```
